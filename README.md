...
### dave.lim(풀스택)
처음 코드를 마주하고 뭐부터 해야할지 방향성을 잡는 데 너무 오래 걸렸습니다.
충분히 배운 내용 안에서 어떤 부분에 병목이 생길 수 있는지 유추할 수 있었는데 너무 코드 자체를 이해하려는 생각 때문에 오히려 어렵게 느껴졌던 것 같습니다.
방향성을 잡지 못해서 일단 코드를 이해하려고 시간을 소모했지만 그조차도 지지부진한 것에 조급함을 느껴 생각을 너무 좁게 했던 것 같습니다.
작업 진행할 때 ai를 정말 많이 사용했는데 단순하게 성능 개선, 코드 개선을 요청했을 때 절대적으로 지켜야 하는 e2e 형식을 벗어나거나 기존 계약을 벗어나게 만드는 결과를 내어서 다시 작업하게 만든 부분이 많았습니다.
이 부분을 돌이켜보면 처음부터 agent.md를 사용해서 기존의 계약을 유지하며 내부 구현을 변경한다거나 구조를 변경해도 e2e 시나리오를 따라가도록 변경하라는 식으로 전체적인 가이드라인을 정하고 작업을 시작했으면 좋았겠다 생각이 듭니다.
그리고 마음이 급해져서 프롬프트 자체도 너무 단순하게 작성해 의도한 작업이 아니었던 적도 있고 위험 징후를 발견했지만 별 거 아니겠지 생각하다가 크게 잘못된 부분을 고치느라 시간을 오래 쓴 부분이 있었습니다. 
ai 사용에 있어서 케빈이 가르쳐 주셨던 부분을 제대로 적용하지 못했기에 이런 문제가 발생했습니다.
아무리 급하더라도 작업을 진행할 때는 절대적인 순서나 절차를 정해서 진행하는 게 좋겠다고 느꼈습니다.
그리고 너무 큰 방향부터 진행하려다 보니 금방 진행할 수 있는 작은 부분의 최적화들을 대부분 처리하지 못했던 것 같아 아쉽습니다.
확실하게 해결할 수 있지만 변경이 적은 부분부터 했다면 최종적으로 해낸 변경이 더 많았을 것 같습니다.
협업 과정에서도 부족한 부분이 많았습니다.
pr을 어떻게 할지 협업, 소통을 어떻게 할지를 구체적으로 정하지 않아서 각자의 작업과 변경이 마구 뒤섞여서 결국 어떤 변경으로 인한 영향인지 확인할 수 없는 상황이 일어났습니다.
찰리의 지적대로 병목 원인을 분석할 때도 프론트가 문제인지 백엔드가 문제인지 인프라가 문제인지 잘 파악하지 못한 상태로 원인에 대한 추측으로 문제를 해결하려 해서 성능이 개선이 되더라도 결국 근본적 원인을 해결하지 못했다고 생각합니다. 이 부분은 결국 문제를 분석할 때도 풀스택답게 프론트와 백엔드가 어떤 흐름으로 진행되고 어떤 부분에서 문제가 생겼는지 관측하고 분석하는 부분이 부족했다고 생각합니다.
추가로 변경 범위를 통제하는 부분이 부족했기에 분석을 제대로 못했다고 생각합니다.
진행이 막혔을 때 멈춰서 왜 막히는지 알고 지나가야 하는데 그냥 ai한테 해줘.. 식으로 시켜서 시간을 낭비한 부분이 많았습니다. 이 부분은 전문가적 태도를 지니지 못한 부분이라고 생각해 아쉬움을 느끼는 부분입니다.
결과적으로 토큰도 많이 쓰고 효율적으로 사용하지 못했습니다.
구체적인 방향은 결국 사람이 정하는 게 중요한데 이번 부하테스트 기간에는 ai를 사용한 작업 지시를 구체적이지 않고 대략적으로만 정한 부분이 아쉽습니다.
애초에 테스트 코드와 e2e테스트가 있어서 문제가 발생하면 직접 어디가 문제인지 파악하는 게 훨씬 쉬웠을 텐데 코드에 대한 이해가 부족하다고 작업을 ai에게 전적으로 맡기는 방향으로 진행하다 보니 너무 주먹구구식 개발을 했던 것 같습니다.
병목 사항 파악하는 능력, 다중 인스턴스에서 어떤 부분이 필요한지, 현재 환경에서 제한되는 부분이 어떤 건지 등을 파악하는 큰 시야의 필요성도 체감했습니다.
아쉬운 부분이 많았지만 경험의 부재로 인해 생긴 문제라고 생각하고 이런 좋은 기회를 경험할 수 있고 좋은 팀원들과 함께할 수 있어서 좋은 시간이었습니다.
